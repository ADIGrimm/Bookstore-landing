...
public class OrderServiceImpl implements OrderService {
    ...
    public OrderDto create(Long userId, CreateOrderRequestDto address) {
        ...
        Set<OrderItem> orderItems = cartItems.stream()
                .map(cartItem -> {
                    bookQuantityService.checkEnoughQuantity(
                            cartItem.getBook().getId(), cartItem.getQuantity()
                    );
                    bookQuantityService.updateQuantity(
                            cartItem.getBook().getId(), -cartItem.getQuantity()
                    );
                    OrderItem orderItem = orderItemMapper.fromCartToOrderItem(cartItem);
                    orderItem.setId(null);
                    orderItem.setOrder(order);
                    Book managedBook = bookRepository.findById(cartItem.getBook().getId())
                            .orElseThrow(() -> new EntityNotFoundException(
                                    "Book not found: " + cartItem.getBook().getId()
                            ));
                    orderItem.setBook(managedBook);
                    return orderItem;
                })
    ...
    public OrderDto updateStatus(Long orderId, UpdateOrderStatusDto updateDto) {
        ...
        if (updateDto.getStatus().equals(Order.Status.CANCELLED)) {
            for (OrderItem item : order.getOrderItems()) {
                bookQuantityService.updateQuantity(item.getBook().getId(), item.getQuantity());
            }
        }
        return orderMapper.toDto(orderRepository.save(order));
    }
